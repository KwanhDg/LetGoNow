...
// Định nghĩa kiểu dữ liệu cho các dịch vụ
interface Service {
  id: number;
  name: string;
  price: number;
...
// Hàm xác nhận thanh toán (giả lập)
async function confirmCheckout() {
  "use server";
  const { data: userData, error: userError } = await supabase.auth.getUser();
  if (userError || !userData?.user) throw new Error('Please login to checkout');

  // Giả lập xác nhận thanh toán (có thể tích hợp API thanh toán thực tế ở đây)
  const { data: bookings, error: bookingError } = await supabase
    .from('bookings')
    .select('*')
    .eq('user_id', userData.user.id);

  if (bookingError) throw new Error('Failed to process checkout');
  if (!bookings || bookings.length === 0) throw new Error('Cart is empty');

  // Xóa giỏ hàng sau khi thanh toán (giả lập)
  await supabase.from('bookings').delete().eq('user_id', userData.user.id);
}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-100 to-white">
      <header className="bg-blue-600 text-white p-4 flex justify-between items-center">
        <h1 className="text-2xl font-bold">LetGoNow - Checkout</h1>
      ...
      <section className="container mx-auto py-12 px-4">
        <h3 className="text-3xl font-semibold text-center text-blue-800 mb-8">Checkout</h3>
        {errorMessage ? (
          ...
          <div className="space-y-6">
            ...
            <div className="text-center mt-6">
              <h4 className="text-2xl font-semibold text-blue-800">Total: ${totalPrice}</h4>
              <form action={confirmCheckout} className="mt-4">
